...
（1）考虑将工资变量（$wage_i$）对三个定性变量（$edu_i,dpt_i,sex_i$）进行**部分混合形式**的虚拟变量线性回归建模（明确要求$sex_i$与$edu_i$的交叉项进入，其他交叉项不进入模型）。请你写出以**\{仅有文盲学历和临时合同的男性\}**工人为基础组的虚拟变量回归总体模型。（**要求**：回归系数参数用$\gamma_1,\gamma_2,\cdots$表示；随机干扰项用$v_i$表示）。
...
（1）考虑将工资变量（$wage_i$）对定量变量（$age_i$,$age^2_i$），以及三个定性变量（$edu_i,dpt_i,sex_i$）进行**部分混合形式**的虚拟变量线性回归建模（明确要求$sex_i$与$edu_i$的交叉项进入，其他交叉项不进入模型）。请你写出以**\{仅有文盲学历和临时合同的男性\}**工人为基础组的虚拟变量回归总体模型。（**要求**：回归系数参数用$\gamma_1,\gamma_2,\cdots$表示；随机干扰项用$v_i$表示）。
