...
（2）建立工作文件（WF），命名为`econometric-lab01`；以及建立工作页（page），命名为`cpi`。

答：此问不用作答，完成指定操作即可！

</br>

（3）将数据（`exercise-Lab01-Table-CPI.xlsx`）导入到刚才建好的Eviews工作文件中。
...
利用所给数据计算每个国家的**年度**通货膨胀率**序列**（series![](pic/object/Series.png)），根据国家的不同，请分别命名为新序列为`inf_japan`、`inf_german`、$\ldots$\。
...
根据所给数据计算每个国家的1980年-2005年间的**平均**通货膨胀率（放在一起），并制作成一张EViews表。（**答题要求**：用EViews计算，并在下面空白处将结果制作成**三线表**）
...
（1）利用所给数据计算7个国家通货膨胀率之间的**相关系数表**，并创建一个EViews表对象（table![](pic/object/Table.png)），命名为`tab_corrl`。最后将该EViews表对象“截图”复制到下面空白处。
