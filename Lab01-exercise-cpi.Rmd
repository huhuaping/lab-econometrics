...
cols_students <- c("dataset", "obs", "year",
                   sort(cols_rdm))
```
...
下面的图\@ref(fig:cpi-show)给出了1980年-2005年7个工业化国家（`r paste0(sort(cols_rdm),collapse = '、')`）的消费价格指数（CPI）数据：
...
>（4）表中CPI指数以（1982-1984=100）。


```{r cpi-show, screenshot.opts = list(selector = ".dataTables_wrapper"),  fig.cap=glue("1980-2005年7个工业化国家的CPI实验数据（n={n}）")}
...
> **温馨提示**：a.通货膨胀率计算公式为：$inf.rate = \frac{CPI_t-CPI_{t-1}}{CPI_{t-1}} 100\%$。b.滞后变量运算的EViews代码可参考`series inf_canada = 100*(1- @lag(cpi_canada, 1)/cpi_canada)`，其中`@lag()`命令表示EVews的滞后变量运算。
