...
```{r}
data_cpi <- as_tibble(openxlsx::read.xlsx("data/exercise-Lab01-Table-CPI.xlsx"))
n <- nrow(data_cpi)
...
（1）请大家下载作业数据文件`exercise-Lab01-Table-CPI.xlsx`到本地电脑。

> **温馨提示**：a.文件尽量不要放在电脑桌面，而是保存在自己清楚的文件夹路径下（如"D://econometrics//my-dataset-file.xlsx"）；b.注意下载工具的使用，不是直接打开xlsx文件，而是要下载到本地电脑，然后再打开！

答：此问不用作答，完成指定操作即可！

</br>

（2）打开EViews软件，创建工作文件（WF），命名为`lab01_cpi`；以及建立工作页（page），命名为`cpi`。
