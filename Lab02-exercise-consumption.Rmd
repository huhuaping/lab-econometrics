...
将数据导入Eviews后，请分别计算出FF原序列![](pic/object/Series.png)：$X_i; Y_i; X^2_i; Y^2_i; X_iY_i$；以及ff离差序列![](pic/object/Series.png)：$(x_i; y_i; x^2_i;y^2_i,x_iy_i)$。在Eviews软件中，以组对象（Group![](pic/object/Group.png)）形式保存，分别命名为`group_ff_upr`和`group_ff_lwr`。最后分别截图到下列空白处。
