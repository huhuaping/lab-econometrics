...
考虑到前述7类模型函数形式，我们希望初步不同形式的自变量（$X_i,ln(X_i),\frac{1}{X_i}$）与因变量（$Y_i,ln(Y_i)$）之间的散点图形态（共有6种散点图组合关系）。请在EViews中，按如下要求分别绘制它们的散点图。
...
（2）（步骤2）：然后，利用矩阵要素相乘的方法（参看计算表\@ref(tab:tbl-calcu)和上述提示），构建并计算1个$(7 \times 1)$的向量对象![](pic/object/Vector.png)，保存并命名为`b_point`，用于存放点弹性的计算量值（参看计算表\@ref(tab:tbl-calcu)）。 请将向量对象![](pic/object/Vector.png)`b_point`的结果复制或截图到如下空白处。
