...
（1）实验目标：序列自相关问题；模型诊断；模型矫正；企业存货案例。

（2）实验任务：题目及答题纸.docx文件下载`lab07-correlation.docx`（请登陆西北农林科技大学在线教育综合平台）[.](lab07-correlation.docx)

（3）实验数据集：.xlsx数据文件下载`lab07-dataset.zip`（请登陆西北农林科技大学在线教育综合平台）[.](dataset/lab07-dataset-`r params$teach_year`/lab07-dataset-`r params$teach_year`.zip)
