...
$$\begin{align}
Y_i&=\hat{\beta}_1+\hat{\beta}_2X_{2i}+\hat{\beta}_3X_{3i}+\hat{\beta}_4X_{4i}+\hat{\beta}_5X_{5i}+u_i && \text{(linear)} (\#eq:mod-linear)
\end{align}$$
...
（1）给定样本外$(x2,x3,x4,x5)=(20,4,4,200)$，请在下列空白处写出矩阵$\boldsymbol{X}_0$的具体形式。
...
> **温馨提示**：需要在Eviews命令窗口中依次运行，1）构造空矩阵`matrix(1,5) x0`；2）给空矩阵赋值：`x0.fill(b=r) 1,20,4,4,200`。
