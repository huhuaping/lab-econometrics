...
考虑到本次实验课内容，EViews操作运算中可能需要保存和命名各类EViews对象，下面表\@ref(tab:obj-multiple)给出了命名规则，供大家在EViews操作中参照使用。
...
运用Eviews菜单（Quick $\Rightarrow$ Estimate Equation），对上述模型\@ref(eq:main)进行回归分析。
...
\begin{align}
Y_i&=\hat{\beta}_1+\hat{\beta}_2X_{2i}+\hat{\beta}_3X_{3i}+\hat{\beta}_4X_{4i}+\hat{\beta}_5X_{5i}+u_i \quad \text{(linear model)} (\#eq:mod-linear)
\end{align}

（1）给定样本回归模型，请写出样本回归模型的矩阵表达式，以及各矩阵的具体形式。
...
（1）请写出线性回归模型\@ref(eq:mod-linear)，斜率系数的矩阵理论计算公式：
...
（1）请写出线性回归模型\@ref(eq:mod-linear)，回归误差方差和标准差的矩阵理论公式：
...
（1）在线性回归模型\@ref(eq:mod-linear)下，请写出平方和分解的矩阵理论表达式。
...
（1）在线性回归模型\@ref(eq:mod-linear)下，请分别写出判定系数$R^2$和调整判定系数$\overline{R}^2$的矩阵理论表达式。
...
F^{\ast} &=\frac{ESS/{f_{ESS}}}{RSS/{f_{RSS}}} =\frac{MSS_{ESS}}{MSS_{RSS}} \\
         &=\frac{(\mathbf{\hat{\beta}X'y}-n\bar{Y}^2)/{k-1}}{{(\mathbf{y'y-\hat{\beta}'X'y})}/{n-k}} \end{align}$$
