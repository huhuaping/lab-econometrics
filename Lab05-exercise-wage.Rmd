...
请你对上述总体回归模型（PRM）的参数（$\beta_0,\beta_1,\ldots,\beta_9$）的理论预期（符号、大小、关系）进行判断，并给出基本的理由。
...
（2）根据EViews的回归方程结果，请将主回归和辅助回归的判定系数结果依次提取出来，将数值添加到向量对象![](pic/object/Vector.png)中，保存并命名该向量对象为：`vec_R2`。最后，补充完成下表。（**要求**：结果保留4位小数）。
...
> **温馨提示**：$VIF_j=1-R^2_j,\quad j\in 1,\cdots,8$。
...
> **温馨提示**：a.可以使用“前向逐步回归法”，或“后向逐步回归法”的任意一种。b.具体操作方法可以参看“实验指导网页”（[网址链接](https://book.huhuaping.com/multilinearity.html)）。
